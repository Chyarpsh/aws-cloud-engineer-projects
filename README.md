# 🌥️ AWS Cloud Engineer Projects & Hands-On Labs

Welcome to my **AWS Cloud Engineering Portfolio** — a collection of practical, hands-on labs, case studies, and troubleshooting documentation aligned with an industry-relevant cloud learning roadmap.

Each lab is documented with screenshots, explanations, errors encountered, troubleshooting steps, and key learning reflections to demonstrate real-world cloud engineering ability.

---

## 🧭 About the Folder Naming (P1, P2, P3...)

| Phase  | Description |
|--------|--------------------------|
| P1 | AWS Basics & Core Concepts |
| P2 | Networking Foundations |
| P3 | Databases & Storage |
| P4 | Cloud Security |
| P5 | Compute & Containers | 
| P6 | DevOps Foundations |
| P7 | AI + Cloud Integrations | 

To avoid confusing nested folders, each lab is stored at the root level and follows this naming format:

P# = Phase Number
Lab## = Lab Number within that phase


Example:  
`p1-lab01-host-a-website-on-amazon-s3` means:

- **p1** → Phase 1 (AWS Basics)  
- **lab01** → Lab #1 within Phase 1  
- **host-a-website-on-amazon-s3** → Short, descriptive lab topic  

---

## 📁 Repository Structure

p1-lab01-host-a-website-on-amazon-s3/
p1-lab02-visualize-data-with-amazon-quicksight/
p1-lab03-cloud-security-with-aws-iam/

p2-lab01-get-hands-on-with-cloud-networking/
p2-lab02-build-a-virtual-private-cloud/
p2-lab03-vpc-traffic-flow-and-security/
p2-lab04-creating-a-private-subnet/
p2-lab05-launching-vpc-resources/
p2-lab06-testing-vpc-connectivity/
p2-lab07-vpc-peering/
p2-lab08-vpc-monitoring-with-flow-logs/
p2-lab09-access-s3-from-a-vpc/
p2-lab10-vpc-endpoints/
p2-lab11-website-delivery-with-cloudfront/

p3-lab01-get-hands-on-with-aws-databases/
p3-lab02-aurora-database-with-ec2/
p3-lab03-connect-a-web-app-with-aurora/
p3-lab04-load-data-into-dynamodb/
p3-lab05-query-data-with-dynamodb/

p4-lab01-cloud-security-with-aws-iam/
p4-lab02-encrypt-data-with-aws-kms/
p4-lab03-threat-detection-with-guardduty/
p4-lab04-secure-secrets-with-secrets-manager/
p4-lab05-build-a-security-monitoring-system/

p5-lab01-apis-with-lambda-and-api-gateway/
p5-lab02-fetch-data-with-aws-lambda/
p5-lab03-build-a-three-tier-web-app/
p5-lab04-deploy-an-app-with-docker/
p5-lab05-deploy-an-app-across-accounts/
p5-lab06-launch-a-kubernetes-cluster/
p5-lab07-set-up-kubernetes-deployment/
p5-lab08-create-kubernetes-manifests/
p5-lab09-deploy-backend-with-kubernetes/

p6-lab01-join-the-7-day-devops-challenge/
p6-lab02-set-up-a-web-app-in-the-cloud/
p6-lab03-connect-a-github-repo-with-aws/
p6-lab04-secure-packages-with-codeartifact/
p6-lab05-continuous-integration-with-codebuild/
p6-lab06-deploy-a-web-app-with-codedeploy/
p6-lab07-infrastructure-as-code-with-cloudformation/
p6-lab08-build-a-ci-cd-pipeline-with-aws/
p6-lab09-create-s3-buckets-with-terraform/
p6-lab10-multi-cloud-data-transfer-with-aws-and-gcp/

p7-lab01-build-a-chatbot-with-amazon-lex/
p7-lab02-add-custom-slots-to-a-lex-chatbot/
p7-lab03-connect-amazon-lex-with-lambda/
p7-lab04-save-user-info-with-a-lex-chatbot/
p7-lab05-set-up-multiple-slots-in-a-lex-chatbot/
p7-lab06-set-up-a-rag-chatbot-in-bedrock/
p7-lab07-chat-with-your-bot-in-the-terminal/
p7-lab08-create-an-api-for-your-rag-chatbot/
p7-lab09-build-a-web-app-for-your-rag-chatbot/
p7-lab10-transcribe-audio-files-with-ai/

---

## 🎯 Learning Goals

- Gain hands-on exposure to foundational and advanced AWS services  
- Build troubleshooting, debugging, and cloud documentation habits  
- Develop real-world, demonstrable project experience  
- Become employment-ready before graduation  

---

## 🧠 Skills Being Developed

- Amazon S3, EC2, IAM, VPC, Route tables, CloudFront  
- DynamoDB, RDS, Aurora  
- Serverless: Lambda, API Gateway  
- Containers: ECR, ECS, Docker, Kubernetes  
- Monitoring & logging: CloudWatch, Flow Logs  
- Security: KMS, Secrets Manager, GuardDuty  
- DevOps pipeline tooling: CodeBuild, CodeDeploy, Terraform  
- AI/Chatbot services: Amazon Lex & Bedrock  

---



